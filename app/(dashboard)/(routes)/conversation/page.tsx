...
async function speakText(text: string | undefined) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      // Call the text-to-speech function here for the bot's response
      ...
      if (error?.response?.status === 403) {
        if (
          ...
          error.response.data === "You've exceeded your pro plan's API limit."
        ) {
          router.push("/settings"); // or wherever you want to redirect
        }
      } else {
